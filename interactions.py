...
Iterated_matrix=np.round(linalg.matrix_power(generosity_matrix(M),np.int(n)),4)  # Eleve la matrice a la puissance n, on arrondit
print("Matrice de connectivite pour k="+str(k)+" elevee a la puissance n="+str(n)+"")
print(Iterated_matrix)
roundish_proba=Iterated_matrix[0,k]         # On extrait le bon coefficient
print("La probabilite d'avoir au moins "+str(k)+" piles consecutifs avec "+str(n)+" lancers est "+str(roundish_proba))
...
plt.title('Probabilite de coopérer au moins $K$ fois consecutives avec '+str(n)+' tours')
plt.show()
...
def tit_for_tat(list_decisions_i, list_decisions_j):
    if len (list_decisions_j) >0:
      return list_decisions_j[-1]  #On prend le dernier élément de la liste du joueur avec lequel on interagit
